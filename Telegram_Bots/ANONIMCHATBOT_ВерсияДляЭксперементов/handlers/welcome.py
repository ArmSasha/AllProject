                ...
                if await check_sub_channels(con.CHANNELS, message.from_user.id):
                    ...
                    if str(referrer_id) != "":
                        if str(referrer_id) != str(message.from_user.id):
                            ...
                            referrals = int(db.get_number_of_referrals(referrer_id))
                            referrals += 1
                            db.set_number_of_referrals(referrer_id, referrals)

                            scores = int(db.get_scores(referrer_id))
                            scores += 50
                            db.set_scores(referrer_id, scores)


                            db.null_like_dislike(message.from_user.id)
                            db.null_messages(message.from_user.id)
                            db.null_scores(message.from_user.id)
                            db.null_number_of_referrals(message.from_user.id)
                            ...
                            await message.reply("Нельзя регестрироваться по своей реферальной ссылке", reply_markup=nav.find_partner)
                    else:
                        db.null_block(message.from_user.id)
