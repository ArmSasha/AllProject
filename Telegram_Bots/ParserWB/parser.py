...
class ParseWB:
    ...
    def parse(self):
        _page = 1
        _limit = 2
        file_name = f"{self.seller_id}_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.csv"
        self.__create_csv(file_name)
        while True:
            response = requests.get(
                f'https://catalog.wb.ru/sellers/catalog?dest=-1257786&limit={_limit}&supplier={self.seller_id}&page={_page}',
            )
            _page += 1
            _limit -= 1
            if _limit == 0:
            ...
            self.__save_csv(file_name, items_info)

    @staticmethod
    def __create_csv(file_name: str):
    ...
    @staticmethod
    def __save_csv(file_name: str, items: Items):
