...
async def payment_command(message: types.Message):
    ...
    await message.answer(
        LOGS_MENU.replace('{userfirstname}', message.from_user.first_name).replace('{logsinformation}', logsinformation_send.strip())\
            .replace('{balance}',formater(data[1])).replace('{userid}',str(message.from_user.id)),
        parse_mode=config.telegram_settings_parse_mode,
...
async def payment_check_command(callback_query: types.CallbackQuery):
    ...
    if method == 'qiwi':
        ...
        SQLite.add_payment(method,payment_id)
        SQLite.add_balance(callback_query.from_user.id, int(float(check.amount)))

        await bot.send_message(
                chat_id=callback_query.from_user.id,
                text=NEW_PAYMENT_TEXT.replace('{balance}',formater(int(float(check.amount)))),
                parse_mode=config.telegram_settings_parse_mode,
                reply_markup=cancel()
            )
...
async def download_file(message: types.Message):
    ...
    if message.document.file_name.split('.')[1] != 'rar' and message.document.file_name.split('.')[1] != 'zip':
        await message.answer(
            text=NO_LOG_FILE_TEXT,
            reply_markup=cancel(),
            parse_mode=config.telegram_settings_parse_mode
        )
    ...
    for _ in range(20):
        file_name += random.choice(list('qwertyuiop1234567890'))

    try:
        SQLite.add_log(upload_logs,file_name + "." + message.document.file_name.split('.')[1])
    except:
        await message.answer(
    ...
    await bot.download_file_by_id(destination=fr"logs/{file_name}." + message.document.file_name.split('.')[1], file_id=file_id)
...
async def talk(message: types.Message):
    ...
    if data[3] == 'lolz_pay':
        ...
        bill_id = random.randint(0,999999999)

        await message.answer(
                text=LOLZ_PAY_TEXT.replace('{amount}',formater(int(message.text))),
                parse_mode=config.telegram_settings_parse_mode,
                reply_markup=check_payment_lolz(payment_id=bill_id, link=f'https://lolz.guru/market/balance/transfer?username={str(lolzaccount["username"])}&from_card=1&amount={message.text}&comment={bill_id}&currency=rub', amount=message.text)
        )
        return
