        ...
        if messages["count"] >= 1:
            ...
            if len(result) == 0:
                ...
                try:
                        ...
                        if kazino == 1:
                            ...
                            if coefficient == 1:
                                ...
                                vk.method("messages.send",
                                          {"peer_id": id, "message": "Вы выиграли " + str(rate * 2) + "!",
                                ...
                                connection.close()
                                vk.method("messages.send",
                                          {"peer_id": id, "message": "Вы выиграли " + str(rate * 3) + "!",
                                           "random_id": random.randint(1, 2147483647)})
                            else:
                                q.execute("SELECT * FROM user_info WHERE User_ID = %s" % (id))
                                result = q.fetchall()
                                ...
                                connection.close()
                                vk.method("messages.send",
                                          {"peer_id": id, "message": "Вы выиграли " + str(rate * 7) + "!",
                                           "random_id": random.randint(1, 2147483647)})
                        elif kazino == 2:
                            vk.method("messages.send",
                                      {"peer_id": id, "message": "Вы проиграли " + str(rate) + ":(",
                                       "random_id": random.randint(1, 2147483647)})
                            connection = sql.connect('user.sqlite', check_same_thread=False)
                            q = connection.cursor()
                            q.execute(
                                "UPDATE user_info SET Balance = '%s' WHERE User_ID = '%s'" % (result[0][3] - rate, id))
                            connection.commit()
                            connection.close()
                    else:
                        vk.method("messages.send",
                    ...
                    vk.method("messages.send", {"peer_id": id, "message": "Введите корректную сумму ставки!",
                                                "random_id": random.randint(1, 2147483647)})
            ...
            try:
                ...
                vk.method("messages.send", {"peer_id": id, "message": "Введите корректный id недвижимости!",
                                            "random_id": random.randint(1, 2147483647)})
