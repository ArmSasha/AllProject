...
class User():
	def __init__(self, minion_level, id, mode, money, pol, energy, level, count5, count10, count15, coal, pers_name, last_minion_work, minion_evo, axe_level, oak, oak_limit, sklad_level, pick_level):
		self.id = id
		self.mode = mode
		self.money = money
		self.pol = pol
...
while True:
	try:
					...
					if msg == 'начать':
						...
						if check_registration(id):
							...
							if not(flag): # если новый пользователь, то переходим в меню регистрации
								users.append( User(id = id, coal = 24, last_minion_work = 0, sklad_level = 0, pick_level = 0, oak_limit = 0, minion_evo = 0, oak = 0, axe_level = 0, pers_name = None, minion_level = 0, mode = 'registration1', money = 100000, pol = None, energy = 100, level = 1, count5 = 0, count10 = 0, count15 = 0) )
									...
									if user.mode == 'start':
										...
										if user.minion_level == 0:
											if msg == 'купить миньёна':
												if user.money >= minion_level_cash[user.minion_evo][user.minion_level]:
													...
													user.minion_level = 1
													sender(id, 'Поздравляем, вы купили миньёна, теперь он будет приносить вам доход!\nВыберите действие:', minion_menu)
												else:
												...
												if user.minion_evo == 0:
													if user.minion_level < 3:
														...
														if user.minion_evo < 1:
															sender(id, 'Ваш миньён достиг максимального уровня, эволюционируйте его!', evo_minion_key)
										...
										if msg == 'улучшить':
											if user.money >= minion_level_cash[user.minion_evo][user.minion_level]:
												user.money -= minion_level_cash[user.minion_evo][user.minion_level]
												user.minion_level += 1
												sender(id, f'Вы успешно улучшили своего миньёна на {user.minion_level} уровень', minion_menu)
												user.mode = 'minion'
											else:
		...
		with open(f'{tdir(__file__)}/errors.txt'.replace('\\', '/').replace('main3.py/', ''), 'a', encoding = 'utf-8') as file:
			file.write(f'{error}\n')
			file.close()