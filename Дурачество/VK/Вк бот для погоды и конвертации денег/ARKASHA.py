...
# session1 = vk.AuthSession(access_token='d4d888d9a3b983ea5c04723f9befac9fa38d633241ed0a95f76b9a92dd7593dd1f28548c0d1b0b608c5e3')
    ...
    if event.type == VkBotEventType.MESSAGE_NEW:
        if event.from_chat:
            ...
            try:
                ...
                #
                #     def get_user_status():
                #         value = vk_api.users.get(user_ids= msg[1], fields='last_seen')
                #         print(value)
                #         sender(id,
                #             value)  # <- получаешь данные вида [{'id': айди, 'first_name': 'Имя', 'last_name': 'Фамилия', 'last_seen': {'time': 1560796737, 'platform': 7}}]
                #         time = datetime.datetime.fromtimestamp(value[0]['last_seen']['time'])
                #         sender(id,value[0]['first_name']['last_name'])
                #         # sender(id,time)
                #
                #
                #     get_user_status()
