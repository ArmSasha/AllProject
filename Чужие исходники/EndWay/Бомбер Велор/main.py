...
async def admin(message: types.Message):
    await message.answer(
        "<b>Админ панель</b>", parse_mode="HTML", reply_markup=admin_kb()
    )
    ...
    if check_sub_channel(await bot.get_chat_member(chat_id=CHANNEL_ID,user_id = message.from_user.id)):
        ...
        if sub != "Отсутствует":
            if (
                ...
                set_default_sub(message.from_user.id)
                user = get_profile(message.from_user.id)
                sub = user[3]
            sub_diff = (
                datetime.datetime.strptime(sub, "%d.%m.%Y %S:%M:%H")
        ...
        await message.answer(f"<b>🐱 Добро пожаловать в Dark Side Bomber 🐱\n👑 Наш бомбер является одним из самых лучших и рентабельных в СНГ!👑\n💪 Наши преимущества 💪:\n👊🏻Бот имеет большое количество смс сервисов  и сервисов для прозвона!\n🎁Розыгрыш подписок каждый месяц в новостном канале!\n👨🏻‍💻Бот и тех.поддержка работают 24/7👨🏻‍💻\n🔧Сервисы постоянно пополняются и обновляются🔧\n💰Возможность бесплатного тестирования функционала👑\n🛡Ваша анонимность при использовании🛡\n📃 Возможность внести свой номер в белый список нашего бомбера\n📈Стоимость подписки составляет демократичные 199₽ - мес.\n✔️Оплата принимается почти на все платежные системы✔️\n⬇️Приобрести доступ можно по кнопке Купить⬇️</b>", parse_mode="HTML", reply_markup=start_kb())
    else:
        await message.answer("Для доступа к функционалу боту, вам нужно подписатся на новостной канал @VelorBomberNews", parse_mode="HTML", reply_markup=sub_kb())
...
async def promo_activate(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await bot.edit_message_text(
            "⚠️ Ошибка, такого промокода нет, либо он уже был активирован ранее",
            message.from_user.id,
            data["message_id"],
            reply_markup=keyboard,
        )
...
async def get_balance_up_amount(message: types.Message, state: FSMContext):
    ...
    await bot.edit_message_text(
        f"<b>💰 Пополнить баланс</b>\nСумма:  <code>{message.text}.00 ₽</code>\n\n🚨 <i>Ссылка на пополнение счета http://qiwi.com/n/NUMBER615<i>\n 🚨 В коментарии к переводу указать ваш телеграмм id, либо ваш юзер нейм",
        message.from_user.id,
        data["message_id"],
        parse_mode="HTML",
    )
...
@dp.message_handler(
    IsBanned(),
    text=["➡️Купить", "🧨Пробный спам", "🔎Информация"],
)
async def main_handler(message: types.Message):
    if message.text == "🧨Пробный спам":
        await message.answer(
            "🧨 Пробный спам\n⬇️⬇️⬇️⬇️⬇️⬇️",
            parse_mode="HTML",
            reply_markup=flood_type(),
        )
        elif message.text == "➡️Купить":

        await message.answer(

            "🏧Автооплата🏧\nСтоимость доступа к боту:\n\n1️⃣ 199р - подписка на месяц\n🔁 999р - подписка на 6 месяцев\n\n🔎Выберите подходящий Вам тариф и оплатите по ссылке снизу, товар выдастся автоматически в виде ссылки в данного бота,которая придет сразу после оплаты,нажмите на нее и подписка активируется автоматически.\n🔧Если у Вас возникли какие-либо проблемы напишите нам - @OctaFlexx / @DarkSideSupporter",
            parse_mode="HTML",
            reply_markup=flood_type(),
        )
        ...
        await message.answer("🔎Информация🔎\n🦾Бот рассылает на все номера по России и СНГ странам, на данный момент сервисов смс рассылки и дозвонов более 50\n📃Возможность внести свой номер в белый список нашего бомбера\n✔️Бот принадлежит каналу: https://t.me/smsbomberat\n🔧Техническая поддержка: @OctaFlexx", parse_mode="HTML", reply_markup=info_kb())
...
<i>Выбери тип флуда</i>""",
        ...
        user = get_profile(callback.from_user.id)
        now = datetime.datetime.now().strftime("%d.%m.%Y %S:%M:%H")
        if callback.data.split('|')[1] != "classic":
            if user[3] == "Отсутствует":
               await callback.answer(
                   "⚠️ Для продолжения необходимо купить подписку", show_alert=True
               )
...
<i>Выбери тип флуда</i>""",
        ...
        await bot.edit_message_text(
            "<b>💵 Подписка</b>\n<i>Выбери наиболее подходящий вариант для тебя из списка ниже</i>",
            callback.from_user.id,
            callback.message.message_id,
            parse_mode="HTML",
        ...
        msg = await bot.edit_message_text(
            "<b>💰 Пополнить баланс</b>\n<i><u>Отправь мне сумму</u> на которую хочешь пополнить баланс или выбери один из вариантов для пополнения баланса</i>",
            callback.from_user.id,
            callback.message.message_id,
...
<i>Для продолжения вам необходимо подтвердить списание средств, нажав кнопку подтверждаю</i>

🆔 ID: <code>{callback.from_user.id}</code>
🔥 Покупка: <code>{rang[0]}</code>
...
Твой баланс:  <code>{user[2]} ₽</code>""",
        ...
        keyboard.add(kb)
        await bot.edit_message_text(
            "<b>🎟 Активировать промокод</b>\n<i>Теперь отправь мне промокод, который ты хочешь активировать</i>",
            callback.from_user.id,
            callback.message.message_id,
            reply_markup=keyboard,
            parse_mode="HTML",
