	...
	if message.text != 'Отмена':
		if message.forward_from_chat != None:
			try:
				...
				bot.send_message(message.chat.id, f'''✖️ Ошибка добавления: <code>{e.result_json["description"]}</code>''',reply_markup=keyboards.main, parse_mode='HTML')	
		else:
			bot.send_message(message.chat.id, '''✖️ Ошибка добавления: <code>Вы не переслали сообщение из канала</code>''',reply_markup=keyboards.main, parse_mode='HTML')	
	else:
		bot.send_message(message.chat.id, '✖️ Вернулись на главную',reply_markup=keyboards.main)
...
def addtovar(message,categor):
	if message.text != 'Отмена':
		msg = bot.send_message(message.chat.id, '''<b>✍️ Укажите описание товара:</b>''',parse_mode='HTML', reply_markup=keyboards.otmena)
...
def addtovar1(message,categor,name):
	if message.text != 'Отмена':
		msg = bot.send_message(message.chat.id, '''<b>✍️ Укажите цену для товара (От 5р):</b>''', parse_mode='HTML', reply_markup=keyboards.main)
