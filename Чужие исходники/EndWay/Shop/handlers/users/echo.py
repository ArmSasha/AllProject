...
@dp.message_handler(lambda message: message.text == "💢 Анти швыр")
async def bot_send_anti(message: types.Message):
    with open("photos/anti.jpg", "rb") as anti:
        await message.answer_photo(photo=anti, caption="📄 <code>Площадка антишвыр откроется в скором веремени...</code>")
...
async def bot_send_city(message: types.Message):
    with open("photos/cartel.jpg", "rb") as cartel:
        await message.answer_photo(photo=cartel, caption="📑 <code>Мы на связи 24/7 для решения ваших вопросов.</code>", reply_markup=support_but())
...
async def bot_send_rayon_info(message: types.Message, state=FSMContext):
    async with state.proxy() as data:
        ...
        with open("photos/cartel.jpg", "rb") as cartel:
            ...
            await message.answer_photo(photo=cartel, caption=f"<b>🌃 Выбран город:</b> <code>{data['city'][2:]}</code>\n"
            f"📦 <b>Выбран товар:</b> <code>{staff[2:]}</code>\n\n"
            "🌆 <i>Напишите (район или улицу вашегр города)</i>\n"
            "🤖 <i>По этим данным наш бот найдет ближайший, актуальный JPS адрес с фото и его описанием.</i>\n\n"
    ...
    try:
        ...
        async with state.proxy() as data:
            if message.text.isdigit():
                ...
                "ℹ️Счёт на оплату действует 30 минут. По истечению этого времени деньги не будут зачислены на ваш баланс!\n\n"
                f"💰 Сумма к оплате:</b> <code>{message.text} руб.</code>",reply_markup=pay_link_button(pay_info[0], pay_info[1], message.text))
                await state.finish()
            else:
